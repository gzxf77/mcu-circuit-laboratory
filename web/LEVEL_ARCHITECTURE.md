...
页面从 URL 的 `level` 参数选择配置；当前只制作课程版第 1 关「让电流走一圈」。它使用直流电阻网络模型，早期 GPIO—电阻—LED 模型保留为未上架的可复用关卡族。以后真正不同的电路可在关卡目录增加配置，同一类电路继续共用模型与交互。
...
当前第 1 关采用 `resistor-dc-v1`，配置电源、接地、两只串联电阻、各自候选值、参考解、探针测点和数值目标。玩家先观察开路与闭合、串联电流和中点电压，再调阻值得到半电源电压。`resistiveNetwork.js` 根据导线连接归并节点，再按 KCL 求节点电压、每只电阻的电流与功率；它不依赖固定接线顺序。关卡数据中的 `referenceOhms` 只用于参考解和测试态，玩家开局使用不同的默认阻值。`seriesConducting` 是可复用的串联电流条件，`completion` 定义通关读数与说明。增加同族关卡时应复用求解器，按目标电路增添元件实例与配置。
