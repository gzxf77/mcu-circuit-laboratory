...
function InventoryIcon({ id }) {
  if (id === 'resistor') return <svg className="inventory-symbol resistor-symbol" viewBox="0 0 40 40" aria-hidden="true">
...
function Scope({ reading, level }) {
  ...
  const maxV = Math.max(4, Math.ceil(level.electrical.gpioHighV * 1.2));
  ...
  return <div className="scope">
    ...
    <svg viewBox="0 0 318 205" role="img" aria-label={reading.pointLabel + '：电压 ' + reading.voltageLabel + '，电流 ' + reading.currentLabel + '。' + reading.detail}>
      ...
      <g className="scope-labels">
        <text x="4" y="162">0</text><text x="4" y="102">{(maxV / 2).toFixed(1)}</text><text x="4" y="63">{level.electrical.gpioHighV.toFixed(1)}</text><text x="4" y="42">{maxV.toFixed(1)}</text>
...
export function App() {
  ...
  const [clearedLevels, setClearedLevels] = useState(() => {
    try {
      const stored = JSON.parse(window.localStorage.getItem('circuit-cleared-v2') || '[]');
  ...
  const requiredPartNames = level.parts.filter(part => !['wire', 'probe'].includes(part.id)).map(part => part.label).join('、');
  ...
  const addPart = (id, position) => {
    ...
    if (!position) return;
    if (id === 'wire') { setMode('wire'); notify('点击两个端点即可连线。'); return; }
    if (id === 'resistor' && !game.resistorOhms) {
      setInventoryPart('resistor');
    ...
    setInventoryPart(null);
    notify((level.parts.find(part => part.id === id)?.label || '元件') + ' 已放置。点击端点连线。');
  };
  ...
  const connect = (port) => {
    if (!pending) { setPending(port); setMode('wire'); return; }
    if (port === pending) { setPending(null); return; }
    if (port.split('.')[0] === pending.split('.')[0]) { notify('请连接到另一个元件的端点。'); setPending(null); return; }
    const key = wireKey(pending, port);
    if (game.wires.some(wire => normalizeWire(wire) === key)) { notify('这两个端点已经连接。'); setPending(null); return; }
    updateGame({ ...game, wires: [...game.wires, pending + '-' + port] });
    setPending(null);
    notify('导线已连接。');
  };
  const reset = () => {
  ...
  const chooseResistor = ohms => {
    updateGame({ ...game, resistorOhms: ohms });
    setInventoryPart(null);
    if (selected === 'resistor' && game.placed.resistor) setSelected('resistor');
  ...
  useEffect(() => {
    try { window.localStorage.setItem('circuit-cleared-v2', JSON.stringify(clearedLevels)); }
  ...
  return <div className="game-shell">
    ...
    <main className="workspace">
      <aside className="left-stack">
        <section className="panel quest-panel">
          ...
          <div className="quest-content">
            ...
            <div className={'goal-summary ' + (report.success ? 'complete' : '')}><Icon icon={report.success ? Check : Target} size={21} weight="bold" /><strong>{report.success ? '目标全部达成 · LED 已点亮' : '当前完成 ' + checks.filter(Boolean).length + ' / ' + checks.length + ' 项'}</strong><small>操作变化时自动更新</small></div>
          </div>
        </section>
        <section className="panel inventory-panel"><div className="panel-title"><Icon icon={Cpu} size={23} /><h2>元件库</h2><small>点击看参数 · 拖入画布</small></div>
          <div className="part-grid">{level.parts.map(part => <button key={part.id} type="button" className={'part-tile ' + ((part.id === 'probe' ? probe : game.placed[part.id]) ? 'placed' : '')}
            ...
            <InventoryIcon id={part.id} /><span>{part.label}</span><small>{(part.id === 'probe' ? probe : game.placed[part.id]) ? '已放置' : part.id === 'resistor' ? (game.resistorOhms ? game.resistorOhms + ' Ω' : '选阻值') : part.count}</small>
          ...
          {inventoryPart && <div className="inventory-popover"><PartParameterMenu id={inventoryPart} level={level} game={game} placement onChooseResistor={chooseResistor} onClose={() => setInventoryPart(null)} onUseWire={() => { setMode('wire'); setInventoryPart(null); notify('连线模式：依次点击两个端点。'); }} /></div>}
        </section>
      </aside>
      <section className="panel board-panel" aria-label="电路搭建区">
        ...
        <div className="board-stage"><CircuitBoard level={level} game={game} componentStates={report.componentStates} currentPath={report.currentPath} flowEdges={report.flowEdges} failureEffect={['overcurrent', 'gpio-short', 'supply-short'].includes(report.kind) ? report.kind : null} probe={probe} probeReading={probeReading} mode={mode} pending={pending} selected={selected} zoom={zoom} onConnect={connect} onProbeChange={setProbe} onSelect={selectBoard} onMove={movePart} onMoveEnd={finishMove} onDropPart={addPart} onRemoveSelected={removeSelected} onFlipLed={flipLed} onChooseResistor={chooseResistor} />
          {!Object.values(game.placed).some(Boolean) && !probe && <div className="empty-board"><Icon icon={Circuitry} size={42} weight="duotone" /><strong>从空白电路开始</strong><span>点击左侧元件查看参数，再拖入 {requiredPartNames}</span><small>点击两个端点连接导线 · 拖入探针即可观察波形</small></div>}
          ...
          {pending && <div className="board-message"><Icon icon={LinkSimple} size={16} />已选起点：{pending}，请选择另一个端点。Esc 取消。</div>}
    ...
    {successOpen && <div className="success-overlay">
      <section ref={dialogRef} tabIndex={-1} onKeyDown={keepFocusInDialog} className="success-dialog" role="dialog" aria-modal="true" aria-labelledby="success-title" aria-describedby="success-detail">
        ...
        <p id="success-detail">GPIO0 经限流电阻驱动 LED，{level.goals.length} 项任务目标全部达成。</p>
        ...
        <div className="success-note"><Icon icon={Check} size={18} weight="bold" />电流符合本关目标范围，LED 正常点亮</div>
