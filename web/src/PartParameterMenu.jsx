...
export function PartParameterMenu({ id, level, game, componentState, placement = false, onChooseResistor, onFlipLed, onRemove, onClose, onUseWire }) {
  ...
  return <div className="parameter-menu" role="group" aria-label={info.title + '参数与操作'} onPointerDown={event => { pointerTrace('menu-down', { id, target: event.target.tagName, x: event.clientX, y: event.clientY }); event.stopPropagation(); }} onClick={event => event.stopPropagation()}>
    ...
    {stateText && <div className={'parameter-status ' + (componentState.state === 'burned' || componentState.state === 'burst' ? 'fault' : '')}>{stateText}</div>}
    {placement && <p>{info.detail}</p>}
    {info.options && optionsOpen && <div className="parameter-options" role="group" aria-label="选择电阻阻值">{info.options.map(ohms => <button key={ohms} type="button" className={game.resistorOhms === ohms ? 'chosen' : ''} aria-pressed={game.resistorOhms === ohms} onClick={() => onChooseResistor(ohms)}>{ohms} Ω</button>)}</div>}
    <div className="parameter-menu-actions">
      {placement ? <>
        {id === 'wire' && <button type="button" onClick={onUseWire}>启用连线</button>}
        <small>{id === 'wire' ? '依次点击两个端点' : '拖动元件卡片到搭建区放置'}</small>
      </> : <>
        ...
        <button type="button" className="remove" onClick={onRemove}>{id === 'wire' ? '删除连接' : '删除元件'}</button>
      </>}
    </div>
  </div>;
}
